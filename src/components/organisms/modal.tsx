...
const Modal = () => {
  ...
  const modals = openedModals.map(modal => {
    ...
    return (
      <div
        key={id}
        className="fixed top-0 left-0 right-0 bottom-0 m-auto z-30 flex justify-center items-center"
      >
        ...
        <div className="absolute border-inherit w-[250px] rounded-2xl overflow-hidden bg-white">
          <button
            className="absolute top-2.5 right-2.5 w-6 h-6 bg-contain bg-center bg-no-repeat bg-[url('/images/x.png')]"
            onClick={() => {
              closeModal(modal)
            }}
          ></button>
          <div className="px-5 py-[50px] text-center">
            ...
            <div>
              <p className="whitespace-pre-line text-sm font-medium text-black-500">
                {content}
              </p>
            </div>
          </div>
